...
//          System.out.println("\t\t\tRatio = " + Long.toBinaryString(ratio) + ", curCount " + curCount);
          max = max(max, same + curCount);
          maxRatioCount = max(curCount, maxRatioCount);
        }
//        System.out.println("i = " + i + ", j = " + j + ", same = " + same + ", vertical = "
//            + vertical + ", max = " + max + ", " + Arrays.toString(points[i])
//            + ", " + Arrays.toString(points[j]));
      } // for loop for j
    }
    return max;
  }
