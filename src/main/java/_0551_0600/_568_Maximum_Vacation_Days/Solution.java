...
public class Solution {
   ...
   * You can only travel among N cities, represented by indexes from 0 to N-1. Initially, you are
   * in the city indexed 0 on Monday.
   ...
   * You totally have K weeks (each week has 7 days) to travel. You can only take flights at most
   * once per day and can only take flights on each week's Monday morning. Since flight time is
   * so short, we don't consider the impact of flight time.
   * For each city, you can only have restricted vacation days in different weeks, given an N*K
   * matrix called days representing this relationship. For the value of days[i][j], it
   * represents the maximum days you could take vacation in the city i in the week j.
   *
   * You're given the flights matrix and days matrix, and you need to output the maximum vacation
   ...
   * Ans = 1 + 1 + 1 = 3.
   *
   * Since there is no flights enable you to move to another city, you have to stay at city 0 for
   * the whole 3 weeks.
   ...
   * You could stay at a city beyond the number of vacation days, but you should work on the extra
      ...
      for (int start = 0; start < cities; start++) {
        ...
        for (int end = 0; end < cities; end++) {
          if (start == end || flights[start][end] == 1) {
            tmp[end] = Math.max(tmp[end], dp[start] + days[end][k]);
          }
        }
      }
      dp = tmp;
